  ...
  <h3>视频上传：</h3>
    <div class="avatar-uploader-box">
      ...
      <div :key="index" class="video-preview" v-for="(item, index) in videoList">
        <video v-if="item.videoLink!=''" v-bind:src="item.videoLink" @mouseover.stop="item.isShowPopup = true" class="avatar">
          您的浏览器不支持视频播放
        </video>
        <video v-else v-bind:src="item.url" @mouseover.stop="item.isShowPopup = true" class="avatar">
          您的浏览器不支持视频播放
        </video>
        <!-- 显示查看和删除的按钮弹窗 -->
        <div
...
// 上传改变
    ...
    // 删除视频
    deleteVideo(index) {
        this.videoList = this.videoList.splice(index);
        this.$emit('videoChange',this.videoList);
    ...
    handleVideoSuccess (res, file) {
      ...
      if (res.code === 200) {
        // 接收视频传回来的名称和保存地址
        this.videoId =this.videoId +','+ res.result.fileId;
        this.videoList.push(res.result);
